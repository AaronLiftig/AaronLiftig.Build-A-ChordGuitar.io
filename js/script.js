...
function getGuitarParameters() {
    ...
    const fretNumParam = document.getElementById("guitar-fretNum-dropdown").value;
    const accidentalTypeParam = document.getElementById("guitar-accidentalType-dropdown").value;

    return {tuningParam, fretNumParam, accidentalTypeParam};
}


function getGuitar(tuning, fretNum) {
    const guitarApp = new GuitarApp({tuning: tuning, numOfFrets: fretNum});
...
function createFret(guitarAxisRow, accidentalType, guitarString, a, b) {
    if (a === 0) {
        populateGuitarAxis(guitarAxisRow, b);
    }
...
function updateGuitarDiv() {
    ...
    let fretNum = parseInt(params.fretNumParam);
    let accidentalType = params.accidentalTypeParam;

    let guitar = getGuitar(tuning, fretNum);
